...
class MOSE_MIX(object):
    def __init__(self, model:nn.Module, buffer, optimizer, input_size, args):
        ...
        self.mix_kd_temp = 3
        self.mix_ce = nn.CrossEntropyLoss(reduction='none')
        self.kd_loss = nn.KLDivLoss(reduction='none')
        self.pdst = nn.PairwiseDistance(p=2)

    def mix_kd(self, pred, target):
        pred = F.log_softmax(pred, dim=1) / self.mix_kd_temp
        target_soft = F.softmax(target, dim=1) / self.mix_kd_temp
        loss = self.kd_loss(pred, target_soft) * (self.mix_kd_temp ** 2)
        return loss
    ...
    def mix_up(self, x1, x2):
        B = x1.size(0)

        lam = Beta(self.mix_up_alpha, self.mix_up_alpha).sample([B])
        lam = lam.to(x1.device)
        idx = torch.randperm(B).to(x1.device)

        x1 = x1.detach()
        x2 = x2[idx].detach()
        lam_ = lam.view(B, 1, 1, 1)
        mixed_x = lam_ * x1 + (1 - lam_) * x2

        return mixed_x.detach(), lam.view(-1, 1), idx
    ...
    def train_any_task(self, task_id, train_loader, epoch):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if len(self.buffer) > 0:
                ...
                with autocast():
                    ...
                    if task_id > 0:
                        ...
                        mix_new_x, lam_new, idx_new = self.mix_up(new_x, new_x)
                        mix_cat_x, lam_cat, idx_cat = self.mix_up(cat_x, cat_x)

                        new_input_size = new_x.size(0)
                        cat_input_size = cat_x.size(0)

                        all_x = torch.cat((new_x, cat_x, mix_new_x, mix_cat_x))
                        ...
                        for i in range(len(feat_list)):
                            ...
                            cat_feat = feat[new_input_size:new_input_size+cat_input_size]
                            new_mix_feat = feat[new_input_size+cat_input_size:new_input_size+cat_input_size+new_input_size]
                            cat_mix_feat = feat[new_input_size+cat_input_size+new_input_size:]

                            new_pred = pred[:new_input_size]
                            cat_pred = pred[new_input_size:new_input_size+cat_input_size]
                            new_mix_pred = pred[new_input_size+cat_input_size:new_input_size+cat_input_size+new_input_size]
                            cat_mix_pred = pred[new_input_size+cat_input_size+new_input_size:]

                            # mix contrastive loss
                            mix_con_loss = 0.
                            # mix_con_loss += (self.mix_kd(new_mix_pred, new_pred.detach()) * lam_new + \
                            #     self.mix_kd(new_mix_pred, new_pred[idx_new].detach()) * (1 - lam_new)).mean()
                            # mix_con_loss += (self.mix_kd(cat_mix_pred, cat_pred.detach()) * lam_cat + \
                            #     self.mix_kd(cat_mix_pred, cat_pred[idx_cat].detach()) * (1 - lam_cat)).mean()
                            mix_con_loss += (self.mix_mse(new_mix_feat, new_feat.detach()) * lam_new + \
                                self.mix_mse(new_mix_feat, new_feat[idx_new].detach()) * (1 - lam_new)).mean()
                            mix_con_loss += (self.mix_mse(cat_mix_feat, cat_feat.detach()) * lam_cat + \
                                self.mix_mse(cat_mix_feat, cat_feat[idx_cat].detach()) * (1 - lam_cat)).mean()
                            

                            # instance-wise contarstive loss
                            ins_loss = sup_con_loss(proj[:new_input_size+cat_input_size], self.ins_t, all_y)

                            # balanced cross entropy loss
                            ce_loss = F.cross_entropy(cat_pred, cat_y)
                            mix_cat_ce_loss = self.mix_ce(cat_mix_pred, cat_y) * lam_cat + \
                                self.mix_ce(cat_mix_pred, cat_y[idx_cat]) * (1 - lam_cat)
                            ce_loss = 2 * (ce_loss + mix_cat_ce_loss.mean())

                            new_pred = new_pred[:, self.new_class_holder]
                            new_mix_pred = new_mix_pred[:, self.new_class_holder]
                            ...
                            mix_new_ce_loss = self.mix_ce(new_mix_pred, new_y_onehot) * lam_new + \
                                self.mix_ce(new_mix_pred, new_y_onehot[idx_new]) * (1 - lam_new)
                            ce_loss += mix_new_ce_loss.mean()
                            ...
                            loss += ins_loss + ce_loss + distill_loss + mix_con_loss
                            ...
                            loss_log['train/mix_con'] += mix_con_loss.item() if mix_con_loss != 0. else 0.
                        ...
                        new_y = new_y.detach()
                        new_input_size = new_x.size(0)
                        all_x = torch.cat((new_x, mix_new_x))
                        ...
                        for i in range(len(feat_list)):
                            ...
                            new_feat = feat[:new_input_size]
                            new_mix_feat = feat[new_input_size:]
                            
                            new_pred = pred[:new_input_size]
                            new_mix_pred = pred[new_input_size:]
                            ...
                            # instance-wise contarstive loss
                            ins_loss = sup_con_loss(proj[:new_input_size], self.ins_t, new_y)

                            # balanced cross entropy loss
                            ce_loss  = F.cross_entropy(new_pred, new_y)
                            mix_new_ce_loss = self.mix_ce(new_mix_pred, new_y) * lam_new + \
                                self.mix_ce(new_mix_pred, new_y[idx_new]) * (1 - lam_new)
                            ce_loss += mix_new_ce_loss.mean()
            ...
            if num_d % self.print_num == 0 or batch_idx == 1:
                print(f"==>>> it: {batch_idx}, loss: ins {loss_log['train/ins']:.2f} + ce {loss_log['train/ce']:.3f} + distill {loss_log['train/distill']:.3f}  + mix_con {loss_log['train/mix_con']:.3f} = {loss:.6f}, {100 * (num_d / self.total_samples)}%")
