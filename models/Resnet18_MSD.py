...
class ResNetMSD(nn.Module):
    ...
    def features(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        fea11 = self.attention_layers[0](out)
        fea11 = fea11 * out
        fea11 = self.feature_alignment_layer[0](fea11).view(out.size(0), -1)

        out = self.layer1[1](out)

        fea12 = self.attention_layers[1](out)
        fea12 = fea12 * out
        fea12 = self.feature_alignment_layer[1](fea12).view(out.size(0), -1)

        out = self.layer2[0](out)

        fea21 = self.attention_layers[2](out)
        fea21 = fea21 * out
        fea21 = self.feature_alignment_layer[2](fea21).view(out.size(0), -1)

        out = self.layer2[1](out)

        fea22 = self.attention_layers[3](out)
        fea22 = fea22 * out
        fea22 = self.feature_alignment_layer[3](fea22).view(out.size(0), -1)

        out = self.layer3[0](out)

        fea31 = self.attention_layers[4](out)
        fea31 = fea31 * out
        fea31 = self.feature_alignment_layer[4](fea31).view(out.size(0), -1)

        out = self.layer3[1](out)

        fea32 = self.attention_layers[5](out)
        fea32 = fea32 * out
        fea32 = self.feature_alignment_layer[5](fea32).view(out.size(0), -1)

        out = self.layer4[0](out)

        fea41 = self.feature_alignment_layer[6](out).view(out.size(0), -1)

        out = self.layer4[1](out)

        fea42 = self.feature_alignment_layer[7](out).view(out.size(0), -1)

        return [fea11, fea12, fea21, fea22, fea31, fea32, fea41, fea42]
    ...
    def final_feature(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        fea4 = self.feature_alignment_layer[7](out).view(out.size(0), -1)
...
def resnet18_msd(nclasses: int, nf: int = 64) -> ResNetMSD:
    """
    ...
    """
    return ResNetMSD(BasicBlock, [2, 2, 2, 2], nclasses, nf)
